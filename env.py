...
class DroneEnv(object):
    ...
    def step(self, action):
        ...
        # if the current position is too far away from the start point
        if (quad_state.x_val >= 400 or quad_state.x_val <= -400)\
                or (quad_state.y_val >= 400 or quad_state.y_val <= -400)\
                or (quad_state.z_val >= 400 or quad_state.z_val <= -400):
            self.dsteps = 0
            done = 1
            # If certain number of steps are completed then stop
    ...
    def Dimage(self):
        ...
        img1d = np.array(frame.image_data_float, dtype=float)
        img1d = img1d*3.5+30 
        img1d[img1d>255] = 255
    ...
    #This will randomly place the objects in the scene.
    ...
    def compute_reward(self, image, quad_state, quad_vel):
        ...
        print('init_dest is {}'.format(init_dest))
